    ...
    protected NetworkBoundResource() {
        ...
        if (shouldFetch()) {
            source = createCall()
                    ...
                    .flatMap(apiResponse -> loadFromDb().toObservable().map(Resource.Companion::success))
                    .doOnError(t -> onFetchFailed())
                    .onErrorResumeNext(t -> {
                        return loadFromDb()
                                .toObservable()
                                .map(data -> Resource.Companion.error(t.getMessage(), data));
        ...
        result = Observable.concat(
                loadFromDb()
                        .toObservable()
                        .map(Resource.Companion::loading)
                        .take(1),
                source
        );
    }
