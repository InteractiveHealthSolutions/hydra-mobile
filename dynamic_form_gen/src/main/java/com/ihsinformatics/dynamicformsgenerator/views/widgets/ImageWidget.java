  ...
  /*  public static final int maxWidth = 2560;
    ...
    public void startCamera() {
       /* ContentValues values = new ContentValues();
        ...
        if(pictureIntent.resolveActivity(context.getPackageManager()) != null) {
            ((Activity) ImageWidget.this.context).startActivityForResult(pictureIntent,
                    getQuestionId());
        }
    }
    ...
    private void checkCameraPermission() {
        ...
        if (hasWriteContactsPermission != PackageManager.PERMISSION_GRANTED) {
            if (!ActivityCompat.shouldShowRequestPermissionRationale((Activity) ImageWidget.this.context,
                    Manifest.permission.CAMERA)) {
                showMessageOKCancel(context, "For capturing images , You need to provide permission to access your camera",
    ...
    public void checkStoragePermission() {
        ...
        if (hasWriteContactsPermission != PackageManager.PERMISSION_GRANTED) {
            if (!ActivityCompat.shouldShowRequestPermissionRationale((Activity) ImageWidget.this.context,
                    Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                showMessageOKCancel(context, "For adding images , You need to provide permission to access your files",
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                ActivityCompat.requestPermissions((Activity) ImageWidget.this.context,
